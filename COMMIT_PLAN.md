...
## Phase L — v0.3.2 (2026-06-13): silent-failure detection in the reviewer prompt

- **What**: two concrete detection bullets added to `karpathy-reviewer.md` **Principle 1** — (a) error fallbacks that return a valid-looking empty (`[]`/`{}`/`0`) indistinguishable from a legitimate-empty result, poisoning a downstream dedup/diff/aggregation; (b) recognisers that silently reroute unrecognised input to a default bucket instead of visibly rejecting it (recognition vs validation collapsed). Surgical prompt-only diff + version bump in `plugin.json` (0.3.1 → 0.3.2).
- **Why P1, not P3**: both describe code the agent *writes* on the error/edge path (an unsurfaced assumption = "surface what was glossed over"), not working code it *removed* (P3). Confirmed by self-review.
